...
class BlockHandle {//block handler用于记录位置和长度信息，表示一个区块范围
 public:
  BlockHandle();
  ...
  void EncodeTo(std::string* dst) const;//放入<offset><size>
  Status DecodeFrom(Slice* input);//解压<offset><size>

  // Maximum encoding length of a BlockHandle
  enum { kMaxEncodedLength = 10 + 10 };//encoding 的最大长度
...
class Footer {//<metaindex_handle><index_handle><padding><magic number>
 public:
  ...
  enum {
    kEncodedLength = 2*BlockHandle::kMaxEncodedLength + 8 //8表示magic number
  };
